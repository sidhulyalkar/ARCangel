...
def _prepend(name: str, *paths: Path) -> None:
    merged: list[str] = []
    for path in paths:
...
def _candidates() -> list[Path]:
    found: list[Path] = []
    try:
        proc = subprocess.run(["ldconfig", "-p"], capture_output=True, text=True, check=False, timeout=15)
...
def prepare_cuda_linker(work: Path = Path("/kaggle/working")) -> Path:
    ...
    if not candidates:
        raise RuntimeError("Could not resolve libcuda.so.1 despite CUDA runtime availability")
    driver = candidates[0]
    ...
    _prepend("LIBRARY_PATH", link_dir, driver.parent, cuda_home / "lib64", cuda_home / "targets/x86_64-linux/lib")
    _prepend("LD_LIBRARY_PATH", link_dir, driver.parent, cuda_home / "lib64", cuda_home / "targets/x86_64-linux/lib")
    ...
    source.write_text('extern "C" int cuInit(unsigned int); int main(){ return (void*)(&cuInit) == nullptr; }\n')
...
if __name__ == "__main__":
    print("Selected CUDA driver:", prepare_cuda_linker())
