...
FINAL C proved that accelerator detection, ARC wheels, ARCangel source, vLLM 0.19.0 and Qwen snapshot discovery all worked. Both candidates then failed during FlashInfer 0.6.6 SM120 JIT at the final host-link step with:

```text
/usr/bin/ld: cannot find -lcuda: No such file or directory
```

The CUDA runtime itself was healthy. The issue was specifically the build-time unversioned `libcuda.so` name requested by `-lcuda`. FINAL D now resolves the live `libcuda.so.1`, creates a writable `libcuda.so` alias, exports `LIBRARY_PATH` and `LD_LIBRARY_PATH`, uses writable FlashInfer JIT directories, attempts to populate CUDA's stub linker directory when permitted, and compiles a real `c++ -lcuda` self-test before starting vLLM.

The reusable preflight implementation is tracked in `scripts/kaggle_cuda_linker_preflight.py`.
...
See `docs/SUBMISSION_RUNBOOK.md` for the full release checklist and research state.
